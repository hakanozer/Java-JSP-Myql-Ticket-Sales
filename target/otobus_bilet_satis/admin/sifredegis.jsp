...
<%
	...
	if (!duzenle_varmi) {
		...
		String esifre = request.getParameter("sifre");
		...
		try {
			...
			ResultSet sifre_r = st.executeQuery("select *from admin where id = '"+ admin_id + "' and  sifre = '" + esifre + "'");
			if (sifre_r.next()) {
				// şifre doğru işlemleri gerçekleştir

				int degistir = st.executeUpdate("update admin set sifre = '"+ ysifre + "' where id = '" + admin_id+ "'");
				if (degistir > 0) {
...
<%
	...
	if (!id_varmi) {
		...
		try {
			...
			ResultSet rsk = st.executeQuery("select *from admin where id = '"+ request.getParameter("id") + "'");
			rsk.next();
