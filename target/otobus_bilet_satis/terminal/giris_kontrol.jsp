...
<%
	...
	if (!kullanici_adi_k && !sifre_k) {
		...
		ResultSet rs = st
				.executeQuery("select *from personel where kullanici_adi = '"
						+ kul_adi + "' and sifresi = '" + kul_sif + "'");

		if (rs.next()) {

			session.setAttribute("personel_id", rs.getString("id"));
			// beni hatırla kontol yapısı
			boolean beni_hatirla_k = (request
					.getParameter("beni_hatirla") == null);
			if (!beni_hatirla_k) {

				Cookie cookie = new Cookie("personel_id",
						rs.getString("id"));
