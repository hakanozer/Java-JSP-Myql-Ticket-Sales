...
<%
	...
	boolean yaz_durum = (request.getParameter("icerik") == null);
	if (!yaz_durum) {
		...
		ResultSet r = st
				.executeQuery("SELECT COUNT(icerik) AS rowcount FROM hakkimizda WHERE isletme_id='"
		...
		if (count != 0) {
			try {
				...
				int yaz = st
						.executeUpdate("UPDATE hakkimizda SET icerik='"
								+ icerik + "' WHERE isletme_id='"
								+ session.getAttribute("yonetici_id")
								+ "'");
		...
		session.setAttribute("iceriks", icerik);
		response.sendRedirect("hakkimizda_kontrol.jsp");
			...
			<section class="wrapper">
				...
				<div class="row">
					...
					<div class="col-md-12">
						...
						<section class="panel">
							<header class="panel-heading">İçerik Ekle</header>
							...
							<form class="form-horizontal bucket-form" action="" method="post">
								<div class="form-group">
									...
									<div class="col-sm-6">
										<textarea class="form-control" name="icerik" id="icerik"
	...
	<script>
		$(document).ready(function(e) {
			...
			$('button[type="submit"]').click(function() {
				if ($('#icerik').val() == "") {
					alert("İçerik Girmeden Kayıt Yapılamaz");
					$('#icerik').focus();
					return false;
				} else {
					return true;
